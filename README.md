...
「今すぐ記録」を実行すると、Spotlight に似たコメント入力ポップアップが表示されます。コメントを入力して Enter を押すと、すぐにウィンドウキャプチャ・テキスト認識・保存が実行されます。コメントは空のまま Enter を押しても保存されます。⌘ + ENTER を押すとコメントは保存せず、入力内容をキーワードとしてキャプチャビューア検索を実行します。Esc を押すとキャンセルになり、保存は行われません。
