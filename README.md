...
- プロンプトで `./YYYY/MM/DD/*.json` の相対 glob パスを参照指示し、CLI が直接ファイルを読む方式（タイムスロット時も同じ glob を使用し、対象時間帯は `{{TIME_RANGE}}` で AI に伝達）
- 既定プロンプトでは、`captureTrigger = manual` の記録を重要ログとして優先的に要約するよう指示される
- 全日モード: `reports/YYYY/MM/DD/report.md` に保存
- タイムスロットモード: `reports/YYYY/MM/DD/report-0800-1200.md` のように時刻ベースのファイル名で保存
- 同日に再生成した場合は、上書き前の内容をファイル更新時刻ベースのバックアップとして保存
- `-p` / `--prompt` が末尾引数の場合、プロンプト文字列を自動的に引数値として注入
- 定時自動生成を有効化すると、設定した時刻（または各タイムスロットの終了時刻）に生成フローが実行される
- **タイムスロット機能**: 1日を複数の時間帯（例: 午前 8-12時、午後 12-18時、夜 18-24時）に分割し、各時間帯の終了時刻に該当範囲のレコードのみで日報を生成。データ量を適切に分割することで密度の高い日報を作成可能
...
3. **定時自動生成** を有効化し、生成モードを選択:
   - **タイムスロットを使用 OFF**: 従来の単一時刻モード。時・分を設定して1日1回生成。
   - **タイムスロットを使用 ON**: 1日を複数の時間帯に分割して生成。各スロットにラベル・開始時刻・終了時刻・有効/無効を設定可能。デフォルトは午前(8-12)・午後(12-18)・夜(18-24)の3スロット。各スロットの終了時刻に自動生成される。
4. **プロンプトテンプレート** を編集。使用可能なプレースホルダ:
   ...
   - `{{JSON_FILE_LIST}}` — `{{JSON_GLOB_PATH}}` と同値（互換用エイリアス）
   - `{{RECORD_COUNT}}` — 対象レコード数
5. 「デフォルトに戻す」で既定テンプレートにリセット可能。
6. **日報保存先** で保存先ディレクトリを指定可能（未指定時は既定パス）。
7. **今すぐ日報生成** ボタンで手動実行（全日レポートとして生成）。各タイムスロット行の ▶ ボタンで、そのスロットの時間帯のみの日報を手動生成することも可能。
...
└── reports/YYYY/MM/DD/
    ...
    ├── report-1800-2400.md              # タイムスロット別レポート（夜の例）
