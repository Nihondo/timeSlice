...
| **今すぐ記録** | Spotlight風コメント入力ポップアップを表示し、Enter後にキャプチャ + OCR + 保存を実行（`captureTrigger: "manual"`）。Escはキャンセル（保存しない）。コメントは `comments` に保存され、空欄Enterは空文字として保存。コメント確定時は元のアプリへフォーカス復帰を試行。ポップアップ表示中に同ショートカットを再実行すると、ポップアップを閉じて終了（保存しない） |
