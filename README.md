...
| **今すぐ記録** | Spotlight風コメント入力ポップアップを表示し、Enter後にキャプチャ + OCR + 保存を実行（`captureTrigger: "manual"`）。Escはキャンセル（保存しない）。コメントは `comments` に保存され、空欄Enterは空文字として保存。ポップアップ表示時に前面アプリの選択テキスト取得を試行し、取得できた場合はコメント欄に初期セット。コメント確定時は元のアプリへフォーカス復帰を試行。ポップアップ表示中に同ショートカットを再実行すると、ポップアップを閉じて終了（保存しない） |
| **日報を生成** | 設定済みの AI CLI で日報を生成 |
| **timeSliceについて...** | 標準のアバウト画面を表示 |
| **終了** (⌘Q) | アプリを終了 |

## キャプチャの仕組み

1. 設定間隔（デフォルト 60 秒）ごとに最前面ウィンドウをキャプチャ
2. 除外アプリ名または除外ウィンドウタイトルに部分一致する場合は、OCR と画像保存を行わずにアプリ名のみ記録（`ocrText: ""`, `hasImage: false`）
3. 除外対象外の場合は Vision Framework で OCR テキストを抽出
4. 直前の記録と重複する場合はスキップ（ハッシュベース重複排除）
5. JSON レコード + PNG 画像（任意）をローカルに保存（JSON には `captureTrigger` として `manual` / `scheduled` を記録し、手動記録時は `comments` も保存）
...
- 日報生成成功時は通知センターへ通知され、通知クリックで `open` コマンドにより保存済みレポートファイルを開ける（手動/定時）
- 日報生成失敗時も通知センターへ失敗通知を送信する（手動/定時）
- 直近の実行結果は `logs/report-last-run.json` に保存される（実行コマンド・追加引数・投入プロンプト・CLI出力・成否・エラー内容）
...
3. **キーボードショートカット** で「今すぐ記録」のショートカットを設定する。
   ...
   - アプリ起動中は他アプリを操作中でもグローバルに実行可能（実行時にコメント入力ポップアップが表示される）
   - コメント入力ポップアップは、前面アプリの選択テキストを初期入力として取り込む（アクセシビリティ権限が必要。未許可時は初回実行時に許可案内を表示。未許可/未対応アプリでは空欄）
   - コメント入力ポップアップ表示中に同ショートカットを再実行すると、ポップアップを閉じてキャンセルされる（保存しない）
4. **起動** セクションで以下を設定する。
...
4. **OCR除外アプリ** にキーワードを追加すると、フロントアプリ名に部分一致したときはOCRを実行せずアプリ名のみ記録。
5. **OCR除外ウィンドウタイトル** にキーワードを追加すると、ウィンドウタイトルに部分一致したときはOCRを実行せずアプリ名のみ記録。
6. **画像も保存する** トグルで PNG 保存の有無を切り替え。
...
├── logs/report-last-run.json            # 日報直近実行ログ（コマンド/プロンプト/出力/成否）
└── reports/YYYY/MM/DD/
