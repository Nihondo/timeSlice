...
テキスト認識処理はすべてデバイス上で完結し、外部サーバーへのデータ送信は行いません。アプリは Dock に表示されず、メニューバーからのみ操作します。
...
「今すぐ記録」を実行すると、Spotlight に似たコメント入力ポップアップが表示されます。コメントを入力して Enter を押すと、すぐにウィンドウキャプチャ・テキスト認識・保存が実行されます。コメントは空のまま Enter を押しても保存されます。Esc を押すとキャンセルになり、保存は行われません。

ポップアップ上部には、記録対象の「アプリ名」と「ウィンドウタイトル」が2行で表示されます（タイトルが取得できない場合は「（タイトルなし）」を表示）。
...
保存されるデータは JSON レコード（常時）と PNG 画像（設定で有効にした場合）です。JSON には記録日時・アプリ名・ウィンドウタイトル・認識テキスト・記録トリガー（定期 or 手動）・コメントが含まれます。
...
一般タブでは次の2種類の権限を同列で確認できます。

- 画面収録（キャプチャ実行に必要）
- 選択テキストアクセス（「今すぐ記録」ポップアップへの選択テキスト自動入力に必要）

各行の「要求」ボタンで権限ダイアログ表示を試行できます。「権限状態を更新」ボタンを押すと最新の状態を再確認できます。
...
認識テキストがこの文字数以下の場合、記録をスキップします（デフォルト 10 文字）。空白画面や短い情報しかないウィンドウを無視するための設定です。
...
このトグルをオンにすると、認識テキストに加えてキャプチャ画像（PNG）も保存されます。画像は 3 日間保持されます。
...
- 手動記録は、左ペイン/右ペインともに時刻の横にインジケータが表示されます。
- 上部コントロール:
  - 時刻ソート（昇順 / 降順）
  - アプリ名フィルタ（ポップアップメニュー）
  - 記録種別フィルタ（すべての記録 / 手動のみ）
  - テキスト検索（ウィンドウタイトル / 認識テキスト / コメント、Enterで確定）
  - 再読み込み
