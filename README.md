...
timeSlice は macOS Sonoma 以降で動作するメニューバーアプリです。作業中に最前面のウィンドウを定期的にキャプチャし、OCR でテキストを抽出して保存します。蓄積されたデータを AI エージェント CLI に渡すことで、その日の作業内容をまとめた日報を自動生成できます。

OCR 処理はすべてデバイス上で完結し、外部サーバーへのデータ送信は行いません。アプリは Dock に表示されず、メニューバーからのみ操作します。
...
6. [設定ガイド](#設定ガイド)
   ...
   - [プロンプトタブ](#プロンプトタブ)
7. [データの保存先](#データの保存先)
8. [画面収録権限の設定](#画面収録権限の設定)
9. [トラブルシューティング](#トラブルシューティング)
10. [ソースからビルド](#ソースからビルド)
...
「今すぐ記録」を実行すると、Spotlight に似たコメント入力ポップアップが表示されます。コメントを入力して Enter を押すと、すぐにキャプチャ・OCR・保存が実行されます。コメントは空のまま Enter を押しても保存されます。Esc を押すとキャンセルになり、保存は行われません。
...
まず、フロントアプリ名またはウィンドウタイトルが除外設定に部分一致するか確認します。一致した場合は OCR テキストと画像保存をスキップし、アプリ名・ウィンドウタイトル等のメタデータのみを記録します。プライベートなウィンドウや機密情報を扱うアプリを除外するのに使います。

除外対象でない場合は、Vision Framework を使ってウィンドウ内のテキストを OCR で抽出します。直前の記録と内容が同じ（ハッシュが一致）場合は重複としてスキップし、内容が変化したときだけ新しいレコードとして保存します。

保存されるデータは JSON レコード（常時）と PNG 画像（設定で有効にした場合）です。JSON には記録日時・アプリ名・ウィンドウタイトル・OCR テキスト・記録トリガー（定期 or 手動）・コメントが含まれます。
...
timeSlice は外部の AI エージェント CLI（`opencode`、`claude`、`gemini` など）を呼び出して日報を生成します。ローカル LLM と連携できる `opencode` のような CLI の利用を推奨します。
...
現在の権限状態が表示されます。許可済みであれば問題ありません。「権限状態を更新」ボタンを押すと最新の状態を再確認できます。

**キーボードショートカット**

「今すぐ記録」のグローバルショートカットを設定します。ショートカット欄をクリックして入力待ち状態にし、⌘ / ⌥ / ⇧ / ⌃ のいずれかの修飾キーと組み合わせてキーを押すと登録されます。Esc を押すと入力をキャンセル、Delete を押すとショートカットを解除します。

設定したショートカットは、他のアプリを操作中でもシステム全体で有効です。ショートカットを押すとコメント入力ポップアップが表示され、前面アプリで選択中のテキストがあれば自動的に入力されます（アクセシビリティ権限が必要です。未許可の場合、初回実行時に許可を促すダイアログが表示されます）。ポップアップが表示されている間に同じショートカットをもう一度押すと、ポップアップが閉じてキャンセルになります。
...
OCR で抽出したテキストがこの文字数以下の場合、記録をスキップします（デフォルト 10 文字）。空白画面や短い情報しかないウィンドウを無視するための設定です。

**OCR 除外アプリ / 除外ウィンドウタイトル**

キーワードを追加すると、フロントアプリ名またはウィンドウタイトルがそのキーワードに部分一致するときに OCR をスキップします。テキストや画像は保存されず、アプリ名・ウィンドウタイトル等のメタデータのみ保存されます。パスワードマネージャーや個人的なメッセージアプリなど、キャプチャしたくないアプリを登録しておくと便利です。

**画像も保存する**

このトグルをオンにすると、OCR テキストに加えてキャプチャ画像（PNG）も保存されます。画像は 3 日間保持されます。
...
呼び出すコマンド名を入力します（例: `claude`、`gemini`、`opencode`）。

**追加引数**

コマンドに渡す引数を設定します。`-p` や `--prompt` を指定すると、プロンプト文字列がその引数の値として渡されます（末尾引数として指定した場合は自動的にプロンプトが注入されます）。
...
過去に App Sandbox が有効だったバージョンをお使いだった場合、旧データは次のパスに残っています。
...
timeSlice はキャプチャのために画面収録権限が必要です。初回起動時に許可ダイアログが表示されますが、表示されなかった場合や誤って拒否した場合は次の手順で手動設定してください。

1. 「システム設定」を開きます。
2. 「プライバシーとセキュリティ」を選択し、「画面収録」に移動します。
