...
- On popup open, tries to prefill the comment field with currently selected text from the frontmost app (AX permission required; first attempt prompts for permission when missing; falls back to empty when unavailable). `FrontmostSelectionTextResolver.resolveSelectedText` retries once via `AXManualAccessibility` when the initial read finds no focused element or empty text — Electron/Chromium apps (VS Code, Slack, etc.) don't build an accessibility tree unless requested by assistive technology, so the plain `kAXSelectedTextAttribute` read is always empty there. Setting `AXManualAccessibility` asks the app to build the tree, but the build happens asynchronously in the target process, so an immediate re-read can still come back empty even after this call. If it does, and the focused element isn't a secure text field (`kAXSecureTextFieldSubrole`), `ClipboardSelectionReader.readSelectedText()` falls back to sending Cmd+C and reading the pasteboard — Capture Now is an explicit user action, so this extra latency/side-effect trade-off (same one PopSnip restricts to its "quick register" flow) is acceptable here, unlike an ambient panel shown on every keystroke. `PasteboardSnapshot` captures/restores every pasteboard item beforehand so the user's clipboard is never left altered; a clipboard history app will see two entries (the copied selection, then the restored original) since the target app performs the copy itself. Known limitation shared with PopSnip: in apps like VS Code, Cmd+C with nothing selected copies the current line, so the fallback can't distinguish "no selection" from "selection unreadable" and may prefill unintended text.
